...
class ModalityEncoder:
    ...
    def encode_products(
        ...
        assert method in (
            "Re-ranking",
            "Retrieval",
        ), 'Only "Retrieval" and "Re-ranking" methods are supported.'
        ...
        if encoded_data is None:
            ...
            encoded_data = self._create_text_embeddings(
                self.text_encoding_schema, batch_size, save_dir
            )
        ...
        if method == "Retrieval":
            for column, encoding in self.aux_encoding_schema.items():
                if encoding == "one_hot":
                    ...
                    modality = (
                        pd.get_dummies(data[column], dtype=int, drop_first=True) * 2 - 1
                    ).to_numpy()
                elif encoding == "geolocation":
                    modality = np.concatenate(
                        (
                            np.vstack(
                                data[column].apply(self._geospatial_to_cartesian)
                            ),
                    ...
                    scaled_data = (
                        2 * ((data[column] - min_value) / (max_value - min_value)) - 1
                    )
                    full_circle_encoding = self._scalar_to_fourier_series(
    ...
    def encode_query(
        self,
        query_text: str,
        aux_data: dict[str, tuple[Any, float]],
        method: str = "Retrieval",
    ) -> np.array:
        """
        ...
        """
        assert (
            aux_data.keys() == self.aux_encoding_schema.keys()
        ), "The query does not comply with the encoding schema"
        assert (
            self.product_data is not None
        ), "The product data hasn't been encoded yet. Please call encode_products()"
        assert method in (
            "Re-ranking",
            "Retrieval",
        ), 'Only "Retrieval" and "Re-ranking" methods are supported.'
            ...
            for key, (value, weight) in aux_data.items():
                ...
                if encoding == "one_hot":
                    unique_values = sorted(
                        self.product_data[key].dropna().unique()
                    )  # because pd.get_dummies sorts values
                    selection, is_negated = value
                    if selection is None or selection == []:
                        modality = np.zeros(
                            len(unique_values)
                        )  # don't do anything if no value was provided
                    else:
                        modality = np.array(
                            [1 if v in selection else -1 for v in unique_values]
                        )
                        if is_negated:
                            modality = -modality
                elif encoding == "binary":
                    ...
                    if value is None:
                        ...
                        farthest_point = self.product_data[key].iloc[
                            np.argmax(distances)
                        ]
                        modality = self._geospatial_encoding(
                            (longitude, latitude), farthest_point
                        )
                        if is_negated:
                            modality = -modality
                elif encoding == "dense":
                    modality = np.zeros(
                        self.num_harmonics * 2 + 4
                    )  # 2 shift dimensions + 2 half-circle dimensions = 4
                    if value is not None:
                        min_value, max_value = (
                            self.product_data[key].min(),
                            self.product_data[key].max(),
                        )
                        if (
                            len(value) < 3
                        ):  # not an interval filter -> use half-circle encoding
                            try:
                                v, is_negated = value
                            except ValueError:
                            ...
                            lower_bound = (
                                2
                                * ((lower_bound - min_value) / (max_value - min_value))
                                - 1
                                - self.epsilon
                            )
                            upper_bound = (
                                2
                                * ((upper_bound - min_value) / (max_value - min_value))
                                - 1
                                + self.epsilon
                            )
                            modality[3:] = self._interval_encoding(
                ...
                encoded_data = np.concatenate(
                    (encoded_data.flatten(), modality * weight)
                )

        return encoded_data

    def encode_result(
        self, search_result: pd.DataFrame, aux_data: dict[str, tuple[Any, float]]
    ) -> None:
        """
        ...
        if "relevance" not in search_result:
            raise ValueError(
                "The search results dataframe must contain the 'relevance' column."
            )
            ...
            if value not in (None, []):
                ...
                if encoding == "dense":
                    if len(value) < 3:
                        ...
                        col = col.apply(
                            lambda x: -1 if lower_bound <= x <= upper_bound else 1
                        )
                        is_ascending = is_negated
                    col_rank = (col.rank(ascending=is_ascending) - 1) / (
                        len(search_result) - 1
                    )
                elif encoding == "geolocation":
                    longitude, latitude, is_negated = value
                    ...
                    distances = self._haversine_distance(
                        points=col.values, ref_point=(longitude, latitude)
                    )
                    farthest_point = col.values[np.argmax(distances)]
                    query_vec = self._geospatial_encoding(
                        (longitude, latitude), farthest_point
                    )
                    ...
                    col_rank = (col.rank(ascending=False) - 1) / (
                        len(search_result) - 1
                    )
                    ...
                    if any([v in col.values for v in selection]):
                        ...
                        col_rank = (col.rank(ascending=is_negated) - 1) / (
                            len(search_result) - 1
                        )
                    else:
                        continue
    ...
    def _load_text_embeddings(self) -> np.array:
        ...
        try:
            ...
            if len(pt_files) > 1:
                print(
                    f"Warning! Multiple .pt files found in the folder. Loaded {pt_files[0]}"
                )
            return text_embeddings.numpy()
        except (FileNotFoundError, ValueError):
            print(
                "The provided text embeddings file is either invalid or does not exist."
            )
            return

    def _create_text_embeddings(
        self,
        text_encoding_schema: dict[str, float],
        batch_size: int,
        save_dir: str = None,
    ) -> np.array:
        def embed_text() -> np.array:
            ...
            remainder = (
                len(self.product_data) % batch_size
            )  # the remaining data points are distributed over chunks
            chunk_start = 0
            embedding_list = []
            for chunk_index in tqdm(range(num_chunks)):
                chunk_end = (
                    chunk_start + batch_size + (1 if chunk_index < remainder else 0)
                )
                column_embeddings = []
                for col in text_encoding_schema.keys():
                    text_batch = self.product_data[chunk_start:chunk_end][
                        col.strip()
                    ].to_list()
                    with torch.no_grad():
                ...
                weighted_embedding = sum(
                    [w * column_embeddings[i] for i, w in enumerate(weights)]
                )
                normalized_embedding = torch.nn.functional.normalize(
                    weighted_embedding, p=2, dim=-1
                )
            ...
            try:
                torch.save(
                    torch.stack(embedding_list),
                    data_dir() / (save_dir or "") / file_name,
                )
            except RuntimeError:
    ...
    def _interval_encoding(self, lower_bound: float, upper_bound: float) -> np.ndarray:
        vector = np.array(
            [
                self._integrate(lower_bound, upper_bound, k + 1)
    ...
    def _centroid_encoding(self, value: float) -> np.ndarray:
        ...
        farthest_vector = self._scalar_to_fourier_series(
            farthest_value, freq=np.pi / 2, num_harmonics=1
        )
        return self._scale_and_shift(vector, farthest_vector)

    def _geospatial_encoding(
        self, query_point: tuple[float, float], farthest_point: tuple[float, float]
    ) -> np.ndarray:
    ...
    def _scale_and_shift(
        input_vector: np.array, farthest_vector: np.array
    ) -> np.ndarray:
    ...
    def _haversine_distance(
        points: np.ndarray[tuple[float, float]], ref_point: tuple[float, float]
    ) -> np.array:
        ...
        lon2, lat2 = np.radians(np.stack(points)[:, 0]), np.radians(
            np.stack(points)[:, 1]
        )
        dlon = lon2 - lon1
        dlat = lat2 - lat1
    ...
    def _scalar_to_fourier_series(
        ...
        sin_cos_interleaved = (
            np.hstack((sin_components * sigmas, cos_components * sigmas))
            .reshape(-1, sin_components.shape[1])
            .T
        )
    ...
    def _integrate(
        self, lower_bound: float, upper_bound: float, k: int
    ) -> tuple[float, float]:
        norm_factor = 1 / (upper_bound - lower_bound)
        return (
            norm_factor
            * quad(
                lambda x: np.sin(k * np.pi * x) * self._sigma(k)[0],
                lower_bound,
                upper_bound,
            )[0],
            norm_factor
            * quad(
                lambda x: np.cos(k * np.pi * x) * self._sigma(k)[0],
                lower_bound,
                upper_bound,
            )[0],
        )
    ...
    def _sigma(self, harmonics: np.ndarray | int, freq: float = np.pi) -> np.ndarray:
        ...
        if len(harmonics) == 1 and harmonics[0] == 1:
            ...
            return np.sin(freq * harmonics / self.num_harmonics) / (
                freq * harmonics / self.num_harmonics
            )
